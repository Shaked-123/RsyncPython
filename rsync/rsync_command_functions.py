...
def copy_file(src_file, dst_file, global_progress_tracker, bandwidth_controller, worker_number=1):
    try:
            ...
            with open(src_file, "rb") as file_to_copy:
                with open(dst_file, "wb") as file_to_write:
                    while True:
                        ...
                        global_progress_tracker.update_track_progress(worker_number, len(data))
...
def copy_directory_contents(src_path, dst_path, global_progress_tracker, bandwidth_controller, worker_number=1):
    try:
        ...
        for item in os.listdir(src_path):
            try:
                ...
                if os.path.isfile(full_src_path):
                    copy_file(full_src_path, full_dst_path, global_progress_tracker, bandwidth_controller, worker_number)
                elif os.path.isdir(full_src_path):
                    # Copy directory contents (and directory if src_path doesn't include a trailing '/'
                    copy_directory_contents(full_src_path, full_dst_path, global_progress_tracker, bandwidth_controller, worker_number)
...
def execute_copy_command(src_path, dst_path, global_progress_tracker, worker_number,
                         bandwidth, copy_command, total_size, update_dst_condition):
    updated_dst_path = get_updated_destination_path(src_path, dst_path, update_dst_condition)
    global_progress_tracker.add_track_progress(worker_number, src_path, dst_path, total_size)
    bandwidth_controller_obj = BandwidthController(bandwidth)
    copy_command(src_path, updated_dst_path, global_progress_tracker, bandwidth_controller_obj, worker_number)


def rsync_command(src_path, dst_path, global_progress_tracker, worker_number, bandwidth=DEFAULT_BANDWIDTH_KB):
    if os.path.exists(src_path):
        if os.path.isfile(src_path):
            # Copy file
            execute_copy_command(src_path, dst_path, global_progress_tracker, worker_number, bandwidth, copy_file,
                                 os.path.getsize(src_path), not os.path.isdir(dst_path))
        elif os.path.isdir(src_path):
            if not os.path.isfile(dst_path):
                # Copy directory contents (and directory if src_path doesn't include a trailing '/'
                execute_copy_command(src_path, dst_path, global_progress_tracker, worker_number,
                                     bandwidth, copy_directory_contents,
